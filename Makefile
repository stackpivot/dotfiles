...
i3:
	sudo pacman -S i3
	ln -vsf ${PWD}/i3/i3.symlink   ${HOME}/.config/i3
	ln -vsf ${PWD}/i3/i3blocks.symlink   ${HOME}/.config/i3blocks
...
test: ## Test this Makefile with docker without Dropbox
	...
	# sudo docker run -ti --name makefiletest -d dotfiles /bin/bash
	@echo "========== make install =========="
	# sudo docker exec makefiletest sh -c "cd ${PWD}; make install"
	sudo docker exec makefiletest sh -c "pacman -S vim --noconfirm"
	@echo "========== make init =========="
	sudo docker exec makefiletest sh -c "cd ${PWD}; make init"
	@echo "========== make vim  =========="
	sudo docker exec makefiletest sh -c "cd ${PWD}; make vim"
	@echo "========== make aur =========="
